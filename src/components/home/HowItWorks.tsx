...
const steps = [
  {
    ...
    title: "Browse Meals",
    description: "Explore authentic homemade dishes from local chefs in your area",
  },
  {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step, index) => (
            <div key={index} className="relative text-center group">
              {/* Connector line - positioned below icons */}
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-24 left-[60%] w-[80%] h-0.5 bg-gradient-to-r from-primary/40 to-primary/10" />
              )}
              
              {/* Step number */}
              <div className="relative mx-auto mb-6">
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-primary-foreground shadow-warm group-hover:scale-110 transition-transform">
                  {step.icon}
                </div>
